...
export function DiscoverShelf({ recipe, onSelect }: DiscoverShelfProps) {
  ...
  // Uses the shared hook — dedupe and pagination logic are now in one place
  const {
    cards,
    fetchNextPage,
    hasNextPage,
    ...
    isError,
    refetch,
  } = useAniListInfiniteQuery<DiscoverCardData>(
    ["shelf_v1", recipe.id],
    (page) => fetchShelfPageAction(recipe.id, page),
    { staleTime: 1000 * 60 * 30, maxPages: 5 }
  );
