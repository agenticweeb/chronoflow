...
interface Props {
  anime: AiringAnime[];
  onSelect?: (anime: AiringAnime) => void;
}

export function AiringCarousel({ anime, onSelect }: Props) {
    ...
    <section className="w-full mb-12">
      ...
      >
        {anime.map((item, i) => (
          ...
          >
            <div className="relative aspect-[2/3] overflow-hidden rounded-xl bg-gray-900 shadow-lg transition-transform duration-200 group-hover:scale-[1.02] group-active:scale-[0.98]">
              <SafeImage
                src={item.coverImage}
                alt={item.title}
                width={230}
