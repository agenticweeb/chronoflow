...
import { Download, Loader2, Sparkles, Image as ImageIcon, Clock, CheckCircle, Zap } from "lucide-react";
...
export function ShareCard({ result }: ShareCardProps) {
  ...
  const franchiseImage = result.franchiseImage || entries[0]?.imageUrl || "";

  // Calculate Viral Metrics: Skipped Episodes & Time Saved
  const skippedEntries = entries.filter((e: any) => e.tier === "skip" || e.isFiller);
  const skippedEpisodes = skippedEntries.reduce((sum: number, e: any) => sum + (e.episodeCount || 1), 0);
  const skippedMinutes = skippedEntries.reduce((sum: number, e: any) => sum + (e.episodeCount || 1) * (e.durationMinutes || 24), 0);
  const savedHours = Math.floor(skippedMinutes / 60);
    ...
    <div className="glass-card p-6 flex flex-col items-center gap-4 border border-chrono-border/30">
      <div className="text-center max-w-md">
        ...
        <p className="text-xs text-chrono-text-dim mt-1">
          Export your optimized path with time-saving stats. Perfect for sharing on Twitter/X, Discord, or Reddit.
        </p>
      </div>
      ...
      <div className="absolute left-[-9999px] top-[-9999px] pointer-events-none select-none">
        ...
        >
          ...
          {/* Header section with Franchise Image */}
          <div className="flex justify-between items-center border-b border-[#1f1f2e] pb-5 mb-6 relative z-10">
            <div className="flex items-center gap-4">
              {franchiseImage && (
                <div className="w-16 h-16 rounded-xl overflow-hidden border-2 border-white/10 shrink-0">
                  <img src={getProxyUrl(franchiseImage)} alt={franchise} className="w-full h-full object-cover" crossOrigin="anonymous" />
                </div>
              )}
              <div>
            ...
            <div className="text-right flex flex-col items-end">
              ...
              <span className="text-[10px] text-zinc-500 tracking-wider font-semibold mt-0.5">aniwatchorder.cc</span>
            </div>
          </div>

          {/* VIRAL BRAG METRICS BLOCK */}
          {savedHours > 0 || skippedEpisodes > 0 ? (
            <div className="my-6 p-5 rounded-xl bg-gradient-to-r from-indigo-500/10 to-purple-500/10 border border-indigo-500/20 flex items-center justify-between relative z-10">
              <div className="flex flex-col items-center text-center w-1/2">
                <span className="text-4xl font-black text-white">{skippedEpisodes}</span>
                <span className="text-xs text-zinc-400 uppercase tracking-wider mt-1">Episodes Skipped</span>
              </div>
              <div className="w-px h-12 bg-white/10 mx-4" />
              <div className="flex flex-col items-center text-center w-1/2">
                <span className="text-4xl font-black text-indigo-400 flex items-center gap-1">
                  <Clock className="w-6 h-6" /> {savedHours}h
                </span>
                <span className="text-xs text-zinc-400 uppercase tracking-wider mt-1">Time Saved</span>
              </div>
            </div>
          ) : (
            <div className="my-6 p-5 rounded-xl bg-emerald-500/10 border border-emerald-500/20 flex items-center justify-center gap-3 relative z-10">
              <CheckCircle className="w-6 h-6 text-emerald-400" />
              <span className="text-lg font-black text-white uppercase tracking-wider">100% Canon Path · Zero Filler</span>
            </div>
          )}
            ...
            {remainingCount > 0 && (
              <div className="bg-[#12121e]/40 border border-dashed border-[#1f1f2e] p-3 rounded-xl text-center">
                ...
                <p className="text-[10px] text-zinc-500 mt-0.5">
                  Explore full dynamic flowchart, skipping rules & notes at <span className="text-indigo-400 font-bold">aniwatchorder.cc</span>
                </p>
              </div>
            )}
          </div>
          ...
          <div className="border-t border-[#1f1f2e] pt-5 flex justify-between items-center relative z-10">
            ...
            <div className="text-right">
              ...
              <span className="text-[9px] text-zinc-500 block mt-0.5 font-semibold">aniwatchorder.cc</span>
            </div>
          </div>
        </div>
      </div>
